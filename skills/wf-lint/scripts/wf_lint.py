...
CÓPIA EMBARCADA (skills/wf-lint/scripts/wf_lint.py) — usada quando não há um
checkout do repositório disponível (ex.: claude.ai sem acesso a pasta). A
fonte da verdade é tools/wf_lint.py na raiz do repositório; se você tem
acesso a ela, prefira rodá-la diretamente. Única diferença desta cópia: os
schemas ficam em scripts/schemas/ (ao lado do script), não em ../schemas/
(um nível acima, como no layout do repositório) — ver check_schemas().

Valida um produto WF (estrutura L0-L5) contra:
  1. Schemas JSON Schema (validação estrutural — jsonschema é obrigatório)
  2. Lints de coerência entre camadas (o "sistema imunológico" do framework)

Uso:
    python wf_lint.py <produto>
    python wf_lint.py <produto> --level commit   # só falha em checks nível commit
...
def check_schemas(product: Path, layers: dict, report: Report):
    import json
    schema_dir = Path(__file__).resolve().parent / "schemas"  # embarcado: ao lado do script, não ../schemas
    msgs, ok = [], True
